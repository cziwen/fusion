...
| **消除规则** | `player.matchEngine.enabledRules` | Checkbox (Set) | 允许实时开启/关闭特定的消除规则（如五连珠、5-3-1金字塔、3x3方块）。 |
...
- **分组管理**: 调试面板支持二级折叠（如 "Spawn Settings"），便于管理后续增加的调试分类。
