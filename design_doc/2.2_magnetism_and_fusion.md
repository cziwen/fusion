...
- **磁吸设置 (Magnetism Settings)**: 点击标题可展开/折叠。
    ...
    - **批次大小 (`magnetBatchSize`)**: 调试参数保留；当前默认值设为 999，实际逻辑默认全量处理候选。
    - **刷新间隔 (`candidateRefreshInterval`)**: 候选人列表更新频率；当前默认 0.0（每帧刷新）。
    - **可见过滤开关 (`enableVisibleCandidateFilter`)**: 当前默认关闭（`false`），即不做可见性裁剪。
