...
- **范围限制**: 仅在 `attractionRadius` (默认 200) 内生效。
...
当吸附发生时，系统执行严格的逻辑校验与坐标修正。为了防止“幽灵方块”现象（方块刚吸附就被消除但仍在视觉上停留一帧），系统采用**逻辑先行**的挂载流程：

- **吸附触发**:
    - **物理碰撞**: `onCollisionStart` 触发吸附请求。
    - **距离吸附**: 距离目标格中心小于 `snapDistance` (默认 6.0) 时自动吸附。
- **逻辑先行挂载 (Logic-First Attachment)**:
    1. **网格占位**: 首先在 `Player` 的逻辑网格（`_occupiedCells`）中占位。
    2. **立即匹配检查**: 在真正添加子组件前，立即运行 `_checkMatches()` 和 `_checkConnectivity()`。
    3. **条件挂载**: 只有在通过上述检查且未被消除的情况下，才执行 `square.removeFromParent()` 和 `add(square)`。
- **对齐规则**: 
    - 融合后的方块成为 `Player` 的子组件。
    - 局部坐标 `LocalPos = (gridX * 20 + 10, gridY * 20 + 10)`（基于 `Anchor.center`）。
    - 局部角度对齐到最近的 $90^\circ$ 倍数。
- **平滑过渡 (视觉连续性)**:
...
- **核心参与规则**: 玩家核心 `(0,0)` **参与**形状匹配（可作为连线或金字塔的一部分），但**永远不可被消除**。
- **消除规则 (Match Rules)**:
    - **五连珠 (Match 5)**: 任意 5 个连续占用的网格位（含核心）连成一条直线（横向或纵向）。
    - **金字塔 (Match Pyramid)**: 6 个连续占用的网格位排列成 1-2-3 的金字塔结构（支持 4 个朝向）。
- **执行时机**: 在方块正式挂载到玩家组件树**之前**触发逻辑检查，确保被消除的方块不会产生视觉残留。
...
- **磁吸设置 (Magnetism Settings)**: 点击标题可展开/折叠。
    - **吸附半径 (`attractionRadius`)**: 控制引力场覆盖的范围（0 - 500，默认 200）。
    - **吸附力度 (`attractionForce`)**: 控制引力的基础强度（0 - 2000，默认 600）。
    - **批次大小 (`magnetBatchSize`)**: 每帧处理的最大方块数（1 - 100，默认 48）。
    - **刷新间隔 (`candidateRefreshInterval`)**: 候选人列表的更新频率（0.01s - 1.0s，默认 0.15s）。
